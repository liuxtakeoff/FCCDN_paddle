...
class SEModule(nn.Layer):
    ...
    def forward(self, x):
        ...
        x_se = x.reshape([x.shape[0], x.shape[1], x.shape[2] * x.shape[3]]).mean(-1).reshape(
...
class FCS(paddle.nn.Layer):
    def __init__(self, num_band, os=16, use_se=False, **kwargs):
        ...
        self.upsample_x2=nn.Sequential(
                        ...
                        # SynchronizedBatchNorm2d(8, momentum=bn_mom), #单GPU下等同于BatchNorm2D
...
class FCCDN_infer(paddle.nn.Layer):
    def __init__(self, num_band, os=16, use_se=False, **kwargs):
        super(FCCDN_infer, self).__init__()
        if os >= 16:
            dilation_list = [1, 1, 1, 1]
            stride_list = [2, 2, 2, 2]
            pool_list = [True, True, True, True]
    ...
    def forward(self, x):
        ...
        # y2 = self.conv_out_class(y2)
        y = self.conv_out(self.upsample_x2(c))
        return paddle.round(F.sigmoid(y))
