...
import paddle
import os
def torch2paddle(torch_path=None,paddle_path=None):
    if torch_path == None:
        torch_path = r"conv_torch.pth"
    if paddle_path == None:
        paddle_path = r"conv_paddle.pdparams"
    torch_state_dict = torch.load(torch_path)
    # torch_state_dict = torch_state_dict['model_state_dict']
